...
public interface ShortLinkActualRemoteService {
     ...
     * @return 短链接批量创建响应
     */
    @PostMapping("/api/shortlink/v1/create-shortlink/batch")
    ...
    Result<Page<ShortLinkPageResDTO>> pageShortLink(@RequestParam("gid") String gid,
                                                     @RequestParam("orderTag") String orderTag,
                                                     @RequestParam("current") Long current,
                                                     @RequestParam("size") Long size);
    //选择传递离散参数（逐个传递 gid、orderTag 等），而非直接传递 ShortLinkPageReqDTO 对象，主要考虑：
    //  远程接口的兼容性：
    //  接口语义更清晰：
...
//    Result<List<ShortLinkGroupCountQueryRespDTO>> listGroupShortLinkCount(@RequestParam("requestParam") List<String> requestParam);
    ...
    void saveRecycleBin(@RequestBody RecycleBinAddReqDTO requestParam);
    ...
    Result<Page<ShortLinkPageResDTO>> pageRecycleBinShortLink(@RequestParam("gidList") List<String> gidList,
                                                               @RequestParam("current") Long current,
                                                               @RequestParam("size") Long size);
    ...
    void recoverRecycleBin(@RequestBody RecycleBinRemoveReqDTO requestParam);
    ...
    Result<ShortLinkStatisticRespDTO> oneShortLinkStats(@RequestParam("fullShortUrl") String fullShortUrl,
    ...
    Result<ShortLinkStatisticRespDTO> groupShortLinkStats(@RequestParam("gid") String gid,
                                                      @RequestParam("startDate") String startDate,
                                                      @RequestParam("endDate") String endDate);
    ...
    Result<Page<ShortLinkStatisticAccessRecordRespDTO>> shortLinkStatsAccessRecord(@RequestParam("fullShortUrl") String fullShortUrl,
                                                                                   @RequestParam("gid") String gid,
                                                                                   @RequestParam("startDate") String startDate,
                                                                                   @RequestParam("endDate") String endDate,
                                                                                   @RequestParam("enableStatus") Integer enableStatus,
    ...
    Result<Page<ShortLinkStatisticAccessRecordRespDTO>> groupShortLinkStatsAccessRecord(@RequestParam("gid") String gid,
                                                                                    @RequestParam("startDate") String startDate,
                                                                                    @RequestParam("endDate") String endDate,
                                                                                    @RequestParam("current") Long current,
                                                                                    @RequestParam("size") Long size);
